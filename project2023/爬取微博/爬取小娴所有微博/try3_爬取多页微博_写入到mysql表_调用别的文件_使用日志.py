...
class WeiboText:
    ...
    def get_data(self,start_page,end_page):
        for page in range(start_page,end_page):
            ...
            logging.info(f"请求地址为 {url}")
            response=requests.get(url=url,headers=self.headers)
            logging.info(response.text)
            logging.info(f"请求状态为{response.status_code}")
            since_id=response.json()["data"]["since_id"]
            logging.info(f"since_id为{since_id}")
            ...
            logging.info(f"当前处理完成第{page}页微博内容")
            time.sleep(2)

        data_list_json={
            "weibo_id":self.weibo_id_list,
...
def write_csv(csv_fiel_name,dataframe):
    ...
    if os.path.exists(v_result_file):
        ...
        header=["微博ID","发布时间","发布地点",'微博内容',"转发次数","评论条数","点赞次数"]
...
def write_mysql(dataframe,bozhu):
    ...
    dataframe.to_sql(name=f'{bozhu}_weibo_text',con=engine,if_exists='replace',index=True)
